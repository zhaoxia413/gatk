...
public final class ContigAlignmentsModifier {
    ...
    private static class GapSplitter {
        ...
        private AlignmentInterval createInterval( final List<CigarElement> elements ) {
            final List<CigarElement> alignmentElements = new ArrayList<>();
            int initialSoftClipLen = alignmentStartContig;
            CigarElement initialHardClip = elements.get(0);
            ...
            alignmentElements.addAll(elements.subList(alignmentStartIdx, alignmentEndIdx));
            int finalSoftClipLen = unclippedContigLen - alignmentEndContig;
            CigarElement finalHardClip = elements.get(elements.size() - 1);
